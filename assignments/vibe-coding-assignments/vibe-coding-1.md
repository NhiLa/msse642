...
I built a **side-by-side interactive security demo** called `BRAC_DEMO`. The app lets the viewer act as one of three users — Alice, Bob, or Admin — by clicking a session switcher in the top navigation bar.
 
The demo is organized into scenarios. For this write-up, I focus on **Scenario 1: User Profile Access**, which demonstrates IDOR (Insecure Direct Object Reference). The viewer selects a target user from a dropdown and clicks **FIRE_REQUEST** to simulate an API call.
 
- The **left panel (Vulnerable Implementation)** calls `GET /api/demo/vulnerable/profile/{id}` with no ownership check. The server returns the full profile — including SSN, salary, email, and phone number — regardless of who is asking. A `200 OK` is returned even when Alice is requesting Bob's private data.
...
Broken Access Control occurs when an application fails to enforce restrictions on what authenticated (or unauthenticated) users are permitted to do. The most common sub-type demonstrated here is **IDOR — Insecure Direct Object Reference** — where an object identifier (a user ID, order number, file name) is exposed in a URL, and the server does not verify that the caller is authorized to access that specific object.
...
**Parler (2021):** Researchers found that post IDs were sequential integers with no access control. Every public post, including deleted ones, could be scraped in order by simply incrementing the ID. Over 70 TB of data including GPS metadata from videos was downloaded.
 
**Toyota Connected (2023):** Toyota's connected car platform exposed vehicle location data and VIN numbers via API calls that did not verify whether the requesting account owned the vehicle. Any authenticated user could query any vehicle.
...
Replit generated four scenarios by default. I only needed one for this assignment. Rather than stripping the others out, I left them in — they strengthen the demo — and scoped my write-up to Scenario 1 only.
...
### Problem 3: Getting a shareable URL
 
The preview inside Replit is not public by default. I had to click **Publish** in the top-right corner to get a live `.replit.app` URL that anyone can access without logging in.
...
- Optus breach analysis: https://www.oaic.gov.au/privacy/privacy-news/optus-data-breach
