...
### Lab
Nessus is not part of the default Kali Linux installation and had to be installed separately. Additionally, Tenable does not provide a native ARM64 package for Nessus, which posed a challenge since this lab runs on a Mac M1 Pro (ARM-based architecture). To work around this, I installed and ran Nessus inside a **Docker container** on Kali Linux using the official `tenable/nessus:latest-ubuntu` image.
